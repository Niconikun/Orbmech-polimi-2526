...
mu_E = astroConstants(14);      % Sun's gravitational parameter [km^3/s^2];
R_E = astroConstants(24);  % Earth radius [km]
J2_E = astroConstants(34); % Earth J2
textureImage = imread('MarsTexture.jpg');
ToF = 1*3600 + 58*60 + 45.6;      % Time in [s];
...
s1 = [-1964.809 2821.834 596.808 -2.902 -2.044 0.107];
s2 = [4836.089 -6945.559 -1468.959 1.629 1.147 -0.061];



%% Initial position & Orbit Propagation
kep_1 = cart2kep(s1(1:3), s1(4:6),mu_E);
...
kep_2 = cart2kep(s2(1:3), s2(4:6),mu_E);
...
[A,P,E,~,VI,VF,TPAR,THETA] = lambertMR(s1(1:3), s2(1:3), ToF, mu_E, orbitType, Nrev, Ncase, optionsLMR);
fprintf("Applied Battin's Algorithm...");
fprintf('Initial velocity vector: [%f, %f, %f]\n', VI(1), VI(2), VI(3));
fprintf('Final velocity vector: [%f, %f, %f]\n', VF(1), VF(2), VF(3));

y_transfer = [s1(1:3)'; VI'];
...
cost_1 = norm(VI-s1(4:6)');
cost_2 = norm(VF-s2(4:6)');
